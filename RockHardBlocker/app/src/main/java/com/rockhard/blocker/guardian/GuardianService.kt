...
class GuardianService : AccessibilityService() {
        ...
        if (alpha > 0) {
            if (!isTintShowing) {
                val params = WindowManager.LayoutParams(
                    ...
                    WindowManager.LayoutParams.FLAG_FULLSCREEN or WindowManager.LayoutParams.FLAG_LAYOUT_IN_SCREEN or WindowManager.LayoutParams.FLAG_LAYOUT_NO_LIMITS or 
    ...
    private fun triggerBossInvasion(debugReason: String, canDefend: Boolean) {
        ...
        val bossName = if (isGamers) {
            BossNameEngine.generateBossName(triggerWord)
        } else {
            "The Dark One"
        }
        ...
        val isNightfall = debugReason.contains("Nightfall")
        val isDrastic = debugReason.contains("Dumb Phone") ||
                debugReason.contains("Nuclear") ||
                debugReason.contains("No Internet") ||
                debugReason.contains("No Videos")
        val minsSaved = if (isAdult) {
            kotlin.random.Random.nextInt(15, 28)
        } else {
            kotlin.random.Random.nextInt(10, 20)
        }
        ...
        Handler(Looper.getMainLooper()).post {
            ...
            // --- POSITIVE TOASTY SLIDE-IN OVERLAY ---
            ...
            val toastyView = android.widget.LinearLayout(this).apply {
                ...
                val bgColor = if (isGamers) "#E53935" else "#4CAF50"
                val subColor = if (isGamers) "#FFCDD2" else "#C8E6C9"

                setBackgroundColor(Color.parseColor(bgColor))
                setPadding(48, 48, 48, 48)

                val toastyTitle = if (isGamers) {
                    "⚔️ INVASION: $bossName"
                } else if (isTamper || isNightfall || isDrastic || !canDefend) {
                    "🛡️ MOMENTUM ENHANCED: OVERCOME ${triggerWord.uppercase()}"
                } else {
            ...
            // --- FULL SCREEN DETAILS (ONLY SHOWN IF TOASTY IS TAPPED) ---
            ...
            val fleeLogic: () -> Unit = {
                ...
                if (isGamers) {
                    ...
                    prefs.edit()
                        .putBoolean("FLED_BATTLE", true)
                        .putString("FLED_BOSS", bossName)
                        .putLong("FLEE_LOCKOUT_UNTIL", lockoutTime)
                        .apply()
                    performGlobalAction(GLOBAL_ACTION_HOME)
                } else {
            ...
            if (isLockedOut) {
                ...
                tvTitle?.text = if (isGamers) {
                    "🛑 SYSTEM LOCKED 🛑"
                } else {
                    "🛡️ SYSTEM SECURED 🛡️"
                }

                tvMsg?.text = if (isGamers) {
                    "$debugReason\n\nThis action cannot be bypassed without a System Override."
                } else {
                    "Overcame: $triggerWord\n\nThis system action is secured and cannot be bypassed without a System Override."
                }

                btnDefend?.visibility = View.GONE
                btnYield?.text = if (isGamers) "Retreat" else "Maintain Momentum"
                ...
                if (isGamers) {
                    ...
                    btnDefend?.text = "Claim Momentum"
                    btnYield?.text = "Dismiss"
                    tvTitle?.text = "⚡ MOMENTUM GAINED ⚡"

                    if (isTamper) {
                        tvMsg?.text =
                            "System modification blocked.\nStay focused on your goals."
                    } else if (isDrastic || isNightfall) {
                        tvMsg?.text =
                            "Action Overcome.\n\nStay focused on your goals."
                    } else {
                        tvMsg?.text =
                            "You successfully overcame an urge.\n" +
                ...
                btnDefend?.setOnClickListener {
                    ...
                    if (isGamers) {
                        ...
                        if (!isTamper && !isNightfall && !isDrastic) {
                            MomentumEngine.addEarnedMomentum(
                                prefs,
                                triggerWord,
                                isAdult
                            )
                        ...
                        Toast.makeText(
                            ...
                            "Urge Defeated! Momentum Claimed!",
                            Toast.LENGTH_SHORT
                        ).show()
            ...
            toastyView.setOnClickListener {
                try {
                    windowManager?.removeView(toastyView)
                } catch (e: Exception) {}
            ...
            try {
                ...
                // Auto-yield after 6 seconds if the user ignores the notification.
                Handler(Looper.getMainLooper()).postDelayed({
