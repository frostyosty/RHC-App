...
class GuardianService : AccessibilityService() {
    ...
    override fun onServiceConnected() {
        ...
        // Force Global Home then launch App to bypass Android 10+ background restrictions
    ...
    override fun onAccessibilityEvent(event: AccessibilityEvent?) {
        ...
        val safeDomains = listOf("aistudio", "github", "codespaces", "gab")
        ...
        // INDISSOLUBLE SYSTEM WHITELIST: Guardian ignores these completely!
        val protectedPkgs = listOf("com.android.systemui", "launcher", "nexus", "pixel", "gallery", "camera", "dialer", "contacts", "note", "gab")
        if (protectedPkgs.any { packageName.contains(it, ignoreCase = true) }) return

        // 1. APP PACKAGE OVERCOME (With First-Time Reward!)
...
val rewardKey = "FIRST_OVERCOME_APP_$packageName"
            if (!prefs.getBoolean(rewardKey, false)) {
                ...
                val newPartyStr = if (partyStr.isEmpty()) newBeast else "$partyStr;$newBeast"
                prefs.edit().putString("PARTY_DATA", newPartyStr).apply()
                
                Handler(Looper.getMainLooper()).post {
                    Toast.makeText(this, "You found an orphaned $bName! But careful, don't come back here, there are only demons left lurking!", Toast.LENGTH_LONG).show()
                }
            } else {
        ...
        if (packageName.contains("com.android.settings")) {
            // NEW: Anti-ADB Guard! Blocks access to Developer Options to prevent PC Uninstalls.
        ...
        // 2. WEBSITE OVERCOME (With First-Time Reward!)
...
val bossName = if (getString(R.string.flavor_id) == "rhc") BossNameEngine.generateBossName(triggerWord) else "The Dark One"
            ...
            if (!isGamificationEnabled) { 
                ...
                overlayView?.findViewById<Button>(R.id.btnYield)?.setOnClickListener { fleeLogic() }
            ...
            btnDefend?.setOnClickListener { 
                removeOverlay()
                startActivity(Intent(this, GameActivity::class.java).apply { addFlags(Intent.FLAG_ACTIVITY_NEW_TASK or Intent.FLAG_ACTIVITY_CLEAR_TOP or Intent.FLAG_ACTIVITY_SINGLE_TOP); putExtra("UNDER_ATTACK", true) }) 
            }
            try { windowManager?.addView(overlayView, params); isOverlayShowing = true } catch (e: Exception) {}
        }
    }
    ...
    private fun extractDangerousContext(
        ...
        if (text.contains(word, true)) {
            ...
            var isClickable =
                node.isClickable || node.className?.toString()?.contains("EditText") == true
            while (parent != null &&
                !isClickable
            ) {
    ...
    private fun isNodeDangerousContext(
        ...
        if (text.contains(textToFind, true) ||
            desc.contains(textToFind, true)
        ) {
            var parent = node.parent
            var isClickable =
                node.isClickable || node.className?.toString()?.contains("EditText") == true
