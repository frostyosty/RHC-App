...
internal fun GameActivity.setupBattleControls() {
    ...
    btn1.setOnClickListener {
        if (playerLastStand) executeHumanPunch() else executePlayerMove(party[activePetIndex].move1)
    }
    btn2.setOnClickListener { 
        executePlayerMove(party[activePetIndex].move2) 
    }
    ...
    val btn3 = Button(this).apply {
        ...
        // FIXED: Using btn2 directly avoids the Kotlin scope-shadowing crash!
        layoutParams = btn2.layoutParams 
                ...
                val btn = Button(this).apply {
                    ...
                    setOnClickListener {
                        if (index == activePetIndex) { 
                            ...
                            updateBattleUI(); triggerEnemyCounterAttack() 
                        }
                        dialog.dismiss()
                    }
                }
        ...
        DialogUtils.showCustomDialog(this, "Use Equipped Item", null, true, "", null) { content, dialog ->
            if (pet.eqPots > 0) {
                val btn = Button(this).apply {
                    ...
                    setOnClickListener {
                        ...
                        if (effect == 0) { playerHasInstaKill = true; printLog("\n> 🧪 Potion glows RED! Next attack will INSTA-KILL!") } 
                        ...
                        saveParty(); updatePartyScreen(); updateBattleUI(); triggerEnemyCounterAttack(); dialog.dismiss()
                    }
                }
                content.addView(btn)
            }
            if (pet.eqSprays > 0) {
                val btn = Button(this).apply {
                    ...
                    layoutParams = LinearLayout.LayoutParams(LinearLayout.LayoutParams.MATCH_PARENT, LinearLayout.LayoutParams.WRAP_CONTENT).apply { setMargins(0, 0, 0, 16) }
                    setOnClickListener {
                        pet.eqSprays--; saveParty(); updatePartyScreen(); updateBattleUI()
    ...
    findViewById<Button>(R.id.btnDispatch).setOnClickListener {
        ...
        DialogUtils.showCustomDialog(this, "Dispatch Netbeasts", "Select beasts to deploy to $currentCity:", true, "DEPLOY", null) { content, dialog ->
            ...
            val btnSelectAll = Button(this).apply {
                text = "SELECT ALL"
                setBackgroundResource(R.drawable.bg_btn_accent); setTextColor(Color.WHITE)
                layoutParams = LinearLayout.LayoutParams(LinearLayout.LayoutParams.MATCH_PARENT, LinearLayout.LayoutParams.WRAP_CONTENT).apply { setMargins(0, 0, 0, 16) }
            }
            ...
            for (i in availablePets) {
                ...
                checkBoxes.add(cb); content.addView(cb)
            }
