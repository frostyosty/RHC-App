...
class MainActivity : Activity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (prefs.getBoolean("LAUNCH_GAME_DEFAULT", false) &&
            !intent.getBooleanExtra("FROM_GAME", false)
        ) {
        ...
        WeatherEngine.fetchSilent(this, { city, _, _, _, debugStr ->
            prefs
                .edit()
                .putString(
                    "CURRENT_CITY",
        ...
        findViewById<Button>(R.id.btnStep2).setOnClickListener {
            if (!dpm.isAdminActive(compName)) {
                DialogUtils.showCustomDialog(this, "Step 2: Lock App", "This prevents the app from being uninstalled.", true, "LOCK APP", {
                    startActivity(
        ...
        findViewById<Button>(R.id.btnStep3).setOnClickListener {
            ...
            DialogUtils
                .showCustomDialog(this, "Step 3: Web Filter", "Address copied to clipboard!\n\n→ Find 'Private DNS' in settings.\n→ Select 'Custom provider'.\n→ Paste the text.", true, "OPEN SETTINGS", {
        ...
        if (btn4 != null && isChinesePhone) {
            ...
            btn4.text = "STEP 4: AUTOSTART"
            btn4.setOnClickListener {
                prefs.edit().putBoolean("STEP4_CLICKED", true).apply()
                DialogUtils.showCustomDialog(this, "Step 4: Autostart", "Find app in list. Set 'Autostart' to ON.", true, "FIX NOW", {
                    try {
                        startActivity(
                            Intent().apply {
                                component =
            ...
            val btn5 =
                Button(this).apply {
                    tag = "BTN_STEP_5"
                    text = "STEP 5: BATTERY SAVER"
                    setBackgroundResource(
                        if (prefs.getBoolean("STEP5_CLICKED", false)) R.drawable.bg_btn_success else R.drawable.bg_btn_standard,
                    )
                    ...
                    setOnClickListener {
                        prefs.edit().putBoolean("STEP5_CLICKED", true).apply()
                        GuardianService.pauseUntil =
                            System.currentTimeMillis() + 60000L
                        DialogUtils
                            .showCustomDialog(this@MainActivity, "Step 5: Battery Saver", "Tap 'Battery Saver'. Select 'No Restrictions'.\n\n(Rock Hard Shield has been paused for 60 seconds).", true, "FIX NOW", {
                                startActivity(
                                    Intent(Settings.ACTION_APPLICATION_DETAILS_SETTINGS).apply {
                                        data =
                                            Uri.parse("package:$packageName")
                ...
                runOnUiThread {
                    ...
                    if (!isDone) {
                        ...
                        Toast
                            .makeText(
                                this,
                                "SYSTEM SECURED! Legendary Netbeast '${legendary.split(",")[0]}' Unlocked!",
                                Toast.LENGTH_LONG,
                            ).show()
                        refreshUI()
                    } else {
        ...
        findViewById<Button>(R.id.btnAddWeb).setOnClickListener {
            addBlockItem(
                "BLOCKLIST_WEB",
                findViewById<EditText>(R.id.etCustomWeb)
                    .text
                    .toString()
                    .trim()
                    .lowercase(),
        ...
        findViewById<Button>(R.id.btnAddApp).setOnClickListener {
            val pkg =
                when (spinApps.selectedItem.toString()) {
        ...
        btnGame.text = if (BuildConfig.FLAVOR.contains("gamers")) "OPEN NETBEAST SAFARI" else "OPEN RHC MOMENTUM"
        
        btnGame.setOnClickListener {
            val targetClass = when (BuildConfig.FLAVOR) {
                "gamersMaleNetbeasts" -> GameActivity::class.java
                "gamersFemaleHomevisits" -> GameActivity::class.java // Placeholder until Homevisits is built!
                "timesaversMaleMomentum", "timesaversFemaleMomentum" -> GameActivity::class.java // Placeholder until Momentum is built!
    ...
    private fun addBlockItem(
        ...
        if (word.isEmpty() ||
            protectedWords.any { word.contains(it) }
        ) {
            Toast.makeText(this, "Invalid or protected keyword!", Toast.LENGTH_LONG).show()
            return
        }
...
private fun openSettingsMenu() {
        DialogUtils.showCustomDialog(this, "Settings", null, true, "SAVE", null) { content, dialog ->
            ...
            // 3. NIGHTFALL BUTTON
            val activeStart = prefs.getInt("NIGHTFALL_START", -1)
            val pendingTime = prefs.getLong("NIGHTFALL_APPLY_TIME", 0L)
            var nfText = if (activeStart == -1) "Set Nightfall (Sleep) Schedule" else "Nightfall Active (Click to Change)"
            if (pendingTime > System.currentTimeMillis()) nfText = "Nightfall Update Pending (Takes 24h)"

            val btnNightfall = Button(this).apply {
                text = nfText
                ...
                setOnClickListener {
                    android.app.TimePickerDialog(this@MainActivity, { _, sHour, sMin ->
                        android.app.TimePickerDialog(this@MainActivity, { _, eHour, eMin ->
                            val startMins = (sHour * 60) + sMin
                            val endMins = (eHour * 60) + eMin
                            
                            val isFirstTime = prefs.getInt("NIGHTFALL_START", -1) == -1
                            if (isFirstTime) {
                                prefs.edit().putInt("NIGHTFALL_START", startMins).putInt("NIGHTFALL_END", endMins).apply()
                                Toast.makeText(this@MainActivity, "Nightfall Schedule Locked!", Toast.LENGTH_LONG).show()
                            } else {
                                prefs.edit().putInt("PENDING_NIGHTFALL_START", startMins).putInt("PENDING_NIGHTFALL_END", endMins).putLong("NIGHTFALL_APPLY_TIME", System.currentTimeMillis() + (24 * 60 * 60 * 1000)).apply()
                                Toast.makeText(this@MainActivity, "Change Registered. Takes 24 hours to apply.", Toast.LENGTH_LONG).show()
                            }
                            dialog.dismiss()
                        }, 6, 0, false).apply { setTitle("Select WAKE UP Time") }.show()
                    }, 22, 30, false).apply { setTitle("Select SLEEP Time") }.show()
                }
            }
            ...
            content.addView(btnDoze)
            content.addView(btnNightfall)
    ...
    private fun swapAppIcon(useGameIcon: Boolean) {
        ...
        if (useGameIcon &&
            currentDefault != PackageManager.COMPONENT_ENABLED_STATE_DISABLED
        ) {
    ...
    private fun refreshUI() {
        ...
        val btn4 = findViewById<Button>(R.id.btnStep4)
        val btn5 = findViewById<View>(android.R.id.content).findViewWithTag<Button>("BTN_STEP_5")
        ...
        if (btn4?.visibility == View.VISIBLE &&
            step4Done
        ) {
            btn4.text = "STEP 4: VERIFIED ✔️"
            btn4.setBackgroundResource(R.drawable.bg_btn_success)
        }
        if (btn5 != null && step5Done) {
            btn5.text = "STEP 5: VERIFIED ✔️"
            btn5.setBackgroundResource(R.drawable.bg_btn_success)
        }
