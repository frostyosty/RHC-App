...
class MainActivity : Activity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Track Install Time for Aether Math
        ...
        WeatherEngine.fetchSilent({ city, _, _, _, debugStr -> prefs.edit().putString("CURRENT_CITY", city).putString("DEBUG_API_DATA", debugStr).apply() }, {})
        ...
        findViewById<Button>(R.id.btnStep1).setOnClickListener { if (!isAccessibilityServiceEnabled(this, GuardianService::class.java)) { DialogUtils.showCustomDialog(this, "Step 1: The Guardian", "Android hides this setting for security.\n\n→ Tap 'Downloaded apps' or 'Installed services'.\n→ Find '${getString(R.string.app_name)}'.\n→ Turn the switch ON.", true, "GO TO SETTINGS", { startActivity(Intent(Settings.ACTION_ACCESSIBILITY_SETTINGS)) }) } }
        ...
        if (btn4 != null) {
            if (isChinesePhone) {
                ...
                val btn5 = Button(this).apply { tag = "BTN_STEP_5"; text = "STEP 5: BATTERY SAVER"; setBackgroundResource(if (prefs.getBoolean("STEP5_CLICKED", false)) R.drawable.bg_btn_success else R.drawable.bg_btn_standard); setTextColor(Color.WHITE); layoutParams = LinearLayout.LayoutParams(LinearLayout.LayoutParams.MATCH_PARENT, LinearLayout.LayoutParams.WRAP_CONTENT).apply { setMargins(0, 0, 0, 16) }
                    setOnClickListener { prefs.edit().putBoolean("STEP5_CLICKED", true).apply(); GuardianService.pauseUntil = System.currentTimeMillis() + 60000L; DialogUtils.showCustomDialog(this@MainActivity, "Step 5: Battery Saver", "Tap 'Battery Saver'. Select 'No Restrictions'.", true, "FIX NOW", { startActivity(Intent(Settings.ACTION_APPLICATION_DETAILS_SETTINGS).apply { data = Uri.parse("package:$packageName") }); refreshUI() }) }
                }
        ...
        findViewById<Button>(R.id.btnTestFilter).setOnClickListener {
            ...
            Thread {
                ...
                runOnUiThread {
                    val s4 = prefs.getBoolean("STEP4_CLICKED", false)
                    val s5 = prefs.getBoolean("STEP5_CLICKED", false)
                    val isDone = if (isChinesePhone) s1 && s2 && s3 && s4 && s5 else s1 && s2 && s3

                    if (!isDone) {
                        Toast.makeText(this, "SYSTEM NOT SECURE! Complete all steps above.", Toast.LENGTH_LONG).show()
                        refreshUI()
                        ...
                        val beastName = legendary.split(",")[0]
                        Toast.makeText(this, "SYSTEM SECURED! Legendary Netbeast '$beastName' Unlocked!", Toast.LENGTH_LONG).show()
                        refreshUI()
                    } else refreshUI()
                }
            }.start()
        ...
        findViewById<Button>(R.id.btnAddWeb).setOnClickListener { val word = findViewById<EditText>(R.id.etCustomWeb).text.toString().trim().lowercase(); val protectedWords = listOf("systemui", "launcher", "nexus", "pixel", "gallery", "camera", "dialer", "contacts", "settings", "note"); if (word.isNotEmpty()) { if (protectedWords.any { word.contains(it) }) { Toast.makeText(this, "Cannot block core functions!", Toast.LENGTH_LONG).show(); return@setOnClickListener }; val currentList = prefs.getString("BLOCKLIST_WEB", "") ?: ""; prefs.edit().putString("BLOCKLIST_WEB", "$currentList,$word").apply(); findViewById<EditText>(R.id.etCustomWeb).setText(""); Toast.makeText(this, "Permanently overcome Website: $word", Toast.LENGTH_SHORT).show() } }
        findViewById<Button>(R.id.btnAddApp).setOnClickListener { val selectedApp = spinApps.selectedItem.toString(); val packageKeyword = when (selectedApp) { "YouTube" -> "com.google.android.youtube"; "TikTok" -> "com.zhiliaoapp.musically"; "Instagram" -> "com.instagram.android"; "Snapchat" -> "com.snapchat.android"; "Facebook" -> "com.facebook.katana"; "Reddit" -> "com.reddit.frontpage"; "Twitter / X" -> "com.twitter.android"; "Discord" -> "com.discord"; "Twitch" -> "tv.twitch.android.app"; "Telegram" -> "org.telegram.messenger"; "Tinder" -> "com.tinder"; "WeChat" -> "com.tencent.mm"; else -> "" }; val protectedWords = listOf("systemui", "launcher", "nexus", "pixel", "gallery", "camera", "dialer", "contacts", "settings", "note"); if (packageKeyword.isNotEmpty()) { if (protectedWords.any { packageKeyword.contains(it) }) { Toast.makeText(this, "Cannot block core functions!", Toast.LENGTH_LONG).show(); return@setOnClickListener }; val currentList = prefs.getString("BLOCKLIST_APP", "") ?: ""; prefs.edit().putString("BLOCKLIST_APP", "$currentList,$packageKeyword").apply(); Toast.makeText(this, "Permanently overcome App: $selectedApp", Toast.LENGTH_SHORT).show() } }
        findViewById<Button>(R.id.btnGame).setOnClickListener { startActivity(Intent(this, GameActivity::class.java)) }
        findViewById<Button>(R.id.btnDebugConsole).setOnClickListener { DialogUtils.showCustomDialog(this, "Developer Console", "Data saved.", false, "CLOSE", null) }
    ...
    private fun openSettingsMenu() {
        DialogUtils.showCustomDialog(this, "Settings", null, true, "SAVE", null) { content, dialog ->
            ...
            val btnGps = Button(this).apply { text = if (hasGps) "GPS Enabled (Rescues Active) ✔️" else "Grant GPS Permission (Enables Rescues)"; setBackgroundResource(if (hasGps) R.drawable.bg_btn_success else R.drawable.bg_btn_accent); setTextColor(Color.WHITE); layoutParams = LinearLayout.LayoutParams(LinearLayout.LayoutParams.MATCH_PARENT, LinearLayout.LayoutParams.WRAP_CONTENT).apply { setMargins(0, 24, 0, 0) }; isEnabled = !hasGps; setOnClickListener { requestPermissions(arrayOf(Manifest.permission.ACCESS_FINE_LOCATION, Manifest.permission.ACCESS_COARSE_LOCATION), 101); dialog.dismiss() } }

            content.addView(cbGame); content.addView(cbDefault); content.addView(cbVibe); content.addView(btnGps)
    ...
    private fun refreshUI() {
        ...
        if (isPremium) { findViewById<View>(R.id.llOnboarding).visibility = View.GONE; return }
