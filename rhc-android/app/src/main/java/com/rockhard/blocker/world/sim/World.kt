...
class World(val map: WorldMap) {
    ...
    fun addPlayer(ownerId: String, companion: String?, exploreSeconds: Int): Entity {
        val e = Entity(nextId++, EntityKind.PLAYER, map.spawnX, map.spawnY, rng.nextDouble(0.0, Math.PI * 2), "player", 0.7, ownerId = ownerId)
    ...
    private fun spawnBeast(z: Zone): Entity {
        val a = rng.nextDouble(0.0, Math.PI * 2); val r = rng.nextDouble(0.0, z.radius)
        val size = when (z.stage) { 1 -> 0.55; 2 -> 0.75; else -> 1.0 }
        val e = Entity(nextId++, EntityKind.BEAST, map.wrap(z.x + cos(a) * r), map.wrap(z.y + sin(a) * r), rng.nextDouble(0.0, Math.PI * 2), z.species, size, zoneId = z.id)
        pickWaypoint(e, z)
        entities[e.id] = e
        return e
    }
