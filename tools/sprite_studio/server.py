...
class SpriteHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_POST(self):
        if self.path == '/generate':
            ...
            duration = data.get('duration', 125) # Defaults to 8 FPS
            if len(frames) > 0: frames[0].save(os.path.join(SAVE_DIR, filename), save_all=True, append_images=frames[1:], duration=duration, loop=0, disposal=2, transparency=0)
            self.send_response(200); self.send_header('Content-type', 'application/json'); self.end_headers()
            self.wfile.write(json.dumps({'status': 'success', 'file': filename}).encode('utf-8'))
