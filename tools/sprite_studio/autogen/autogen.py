...
so every animation of a beast is guaranteed to be the same creature.
  ...
  python3 tools/sprite_studio/autogen/autogen.py              # everything
  python3 tools/sprite_studio/autogen/autogen.py --only cacheon,titan
  python3 tools/sprite_studio/autogen/autogen.py --anims idle,attack
  python3 tools/sprite_studio/autogen/autogen.py --skip-existing
  python3 tools/sprite_studio/autogen/autogen.py --preview out.png
...
def draw(name, pose):
    d = DESIGNS[name]
    p = pk.Painter(mirror=pose.front)
    d.fn(p, pose)
...
def a_idle(d, n):
    frames = []
    for i in range(8):
        pose = Pose(flap=[0, -1, 0, 1][i % 4], eyes='closed' if i == 6 else 'open', t=i)
...
def a_walk(d, n, front=False):
    frames = []
    for i in range(8):
        pose = Pose(front=front, step=i % 4, flap=[-1, 0, 1, 0][i % 4], t=i)
        img = draw(n, pose)
        if d.float:
...
def a_walk_front(d, n):
    return a_walk(d, n, front=True)


def a_attack_front(d, n):
    wind = draw(n, Pose(front=True, eyes='angry', flap=-1))
    bite = draw(n, Pose(front=True, eyes='angry', mouth=True, flap=1))
    frames = [
        wind,
...
def main():
    ...
    anims = [a.strip() for a in args.anims.split(',')] if args.anims else ANIMATIONS
